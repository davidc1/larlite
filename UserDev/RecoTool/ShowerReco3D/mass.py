...
vertex  = fin.Get('vertex_numuCC_vertex_tree')
...
for n in scan_v:
    ...
    vertex_v = vertex.vertex_numuCC_vertex_branch
        ...
        for i in xrange(len(shower_v)):
            for j in xrange(i+1,len(shower_v)):
                ...
                if (ip < 40.0):
                    ...
                    print 'dEdx1 : %.02f \t dEdx1 : %.02f'%(dedx1,dedx2)
                    print
...
fig = plt.figure(figsize=(10,10))
plt.hist(dedx,bins=np.linspace(0,10,10))
plt.grid()
plt.show()
