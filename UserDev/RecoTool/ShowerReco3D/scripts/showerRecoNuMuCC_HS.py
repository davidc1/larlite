...
def getShowerRecoAlgModular():
    ...
    for line in fin:
        ...
        dQdsAVG_v.append( q )
        ctr += 1
    dQdsAVG = np.median( np.array(dQdsAVG_v) )
    print 'dQds average = %.02f'%np.average(np.array(dQdsAVG_v))
    print 'dQds median  = %.02f'%np.median(np.array(dQdsAVG_v))
    fin.seek(0)
    for line in fin:
        words = line.split()
        x = float(words[0])
        ...
        dqdx.SetResponse(x,y,z,dQdsAVG/q)

    energy.SetElectronLifetime(1e6) # in us DATA value
    energy.SetRecombFactor(0.596)
    energy.SetElecGain(243.6) # MCC8.0 data
...
protoshoweralg.SetVertexProducer("hc")
